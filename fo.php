...
<html>
  ...
  <body>
    ...
    <br><br><br><br>
  	<form action="fo.php" method="post" class="container">
    	<label>Введите здесь своё имя</label><br><br>
    	<input type="text" name="name"><br><br>
    	<label>Введите здесь свой пароль</label><br><br>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	try {
		$sql = 'SELECT pass FROM forum1 WHERE name = :name';
		$query = $pdo->prepare($sql);
		$query->execute(['name' => $name]);
		$user = $query->fetch(PDO::FETCH_ASSOC);

		if ($user && password_verify($pass, $user['pass'])) {
			setcookie('name', $name, time() + 3600 * 24 * 31, "/");
			echo "
			<h4 class='text'>Вы авторизованы!</h4>";
		}
		else {
			echo "<h4 class='text'>такого пользователя нету</h4>";
		}
